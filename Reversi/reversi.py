...
def main(engines, user_names, scores, game_time, verbose):
    try:
        ...
        err_usr_id = int((err[0] + 1) / 2)  # 0 or 1
        other_id = int((1 - err[0]) / 2)  # 1 or 0
        scores[other_id] += 64
        print("\n- {} ({}) ran out of time!\n".format(user_names[err_usr_id], player[err[0]]))
        print("{} ({}) wins the game! (Current score: {})".format(user_names[other_id], player[err[0] * -1],
                                                                  scores[other_id]))
        return scores
